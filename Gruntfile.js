...
module.exports = function (grunt) {
	...
	function initConfig() {
		function getValidateTasks(allTasks) {
			if (process.env.SESEARCH_FAST === "true") {
				return [];
			}
			if (process.env.SESEARCH_W3C_LOCAL_URL && allTasks) {
				return [
					"jshint",
					"karma:unit",
					// "htmlangular:index",
		...
		grunt.initConfig({
			...
			html2js: {
				options: {
					base: "<%= yeoman.dist %>",
					module: "seSearch.html",
					quoteChar: "'"
				},
				main: {
					src: ["<%= yeoman.dist %>/**/*.html"],
					dest: "<%= yeoman.gen %>/seSearchHtml.js"
				}
			},
	...
	function getBrowsersForTest() {
		if (process.env.SESEARCH_BROWSERS_FOR_TEST) {
			return process.env.SESEARCH_BROWSERS_FOR_TEST.split(",");
		} else {
